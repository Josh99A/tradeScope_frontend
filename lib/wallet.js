...
const normalizeDecimal = (value, maxDecimals) => {
...
export const depositFunds = async ({ amount, asset_id, usd_amount }) => {
  const response = await api.post("/wallet/deposit/", {
    amount: normalizeDecimal(amount, 8),
    asset_id,
    ...(usd_amount !== undefined && usd_amount !== null
      ? { usd_amount: normalizeDecimal(usd_amount, 4) }
      : {}),
  });
...
export const withdrawFunds = async ({
  ...
  formData.append("amount", normalizeDecimal(amount, 8));
  ...
  if (usd_amount !== undefined && usd_amount !== null) {
    formData.append("usd_amount", normalizeDecimal(usd_amount, 4));
  }
